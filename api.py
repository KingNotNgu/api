...
def delete_api_key():
    ...
    try:
        ...
        cursor.execute('DELETE FROM api_keys WHERE api_key = ? AND ip = ?', (api_key, user_ip))
        conn.commit()
        rows_deleted = cursor.rowcount
        conn.close()

        if rows_deleted > 0:
            logger.info(f"API key {api_key} từ IP {user_ip} đã bị xóa.")
            return jsonify({'message': 'API key deleted successfully'}), 200
        else:
            logger.warning(f"Thử xóa thất bại: API key {api_key} không tồn tại.")
